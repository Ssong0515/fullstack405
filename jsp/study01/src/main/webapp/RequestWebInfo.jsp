...
<div class="container mt-5">
    ...
    <ul class="list-group">
<%--        request: jsp가 미리 선언하여 사용자가 사용할 수 있도록 해놓은 내장객체, 요청 데이터를 저장 --%>
<%--        getMethod(): 클라이언트가 서버에 데이터 요청 시, 데이터를 전달한 방식을 출력 --%>
        <li class="list-group-item">데이터 전송 방식: <%= request.getMethod() %></li>

<%--    URL과 URI의 차이: URL은 네트워크 상에서 자원이 어디 있는지 위치를 알려주기 위한 규약, URI 이너넷에 있는 자원을 어디에 있는지 자원 자체를 식별하는 방법 --%>
...
<%--    Protocol(프로토콜): 컴퓨터나 원거리 통신 장비 사이에서 메세지를 주고 받기 위한 통신 규약 --%>
<%--    getProtocol: 현재 사용하는 통신 규약 --%>
...
<%--    getServerPort: 현재 접속한 서버에서 사용하는 port 번호를 출력 --%>
        <li class="list-group-item">서버포트: <%= request.getServerPort() %></li>
<%--    getRemoteAddr: 현재 서버에 접속에 사용한 클라이언트의 IP주소를 출력--%>
        <li class="list-group-item">클라이언트 IP 주소: <%= request.getRemoteAddr() %></li>
<%--    getQueryString: URI 뒤에 있는 ? 기호와 함께 전달된 데이터를 의미, GET 방식 사용 시 URL과 함께 전달된 데이터를 출력 --%>
