...
<%
    ...
    try {
//        다운로드를 위해 파일 객체 생성, 저장된 파일명을 기준으로 생성
...
//            인터넷 익스플로러 사용 시 한글 깨짐 처리
            originalFileName = new String(originalFileName.getBytes("UTF-8"), "iso-8859-1");
        } else {
            originalFileName = new String(originalFileName.getBytes("KSC5601"), "iso-8859-1");
        }

//        파일 다운로드를 위해서 서버의 응답 객체인 response 초기화
        response.reset();
//        response 객체의 컨텐츠 타입 설정, 파일 전송 방식으로 설정
        response.setContentType("application/octet-stream");
//        response 객체에 파일 다운로드 설정
...
//        inputStream에 저장된 파일 정보를 byte 단위로 읽어서 OutputStream에 쓰기
