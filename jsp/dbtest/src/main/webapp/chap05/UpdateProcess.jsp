...
<div class="container mt-5">
    ...
    <%
        // 데이터베이스로 sql 쿼리문을 전달하는 객체(Statement) 생성
        Statement stmt = null;
        // sql Update문 생성
        // 자바 소스 코드 내에서 sql 쿼리문을 직접 사용할 경우 sql 쿼리문의 문자열이 길기 때문에
        // += 연산자를 사용하여 여러줄의 sql 쿼리문을 연결해서 사용하게 됨
        // 문자열을 연결해서 사용 시, 문자열의 끝에 공백기호 하나 추가하는 것이 좋음
        String sql = "UPDATE member SET pass = '" + userPw + "', name = '" + userName + "' ";
        sql += "WHERE id = '" + userId + "' ";

        // 데이터베이스는 자바 프로그램의 외부에 존재하는 리소스이므로 반드시 try~catch를 사용
        try {
            // Connection 클래스 타입의 객체를 사용하여 Statement 객체를 생성
            stmt = conn.createStatement();
            // Statement 객체를 통해서 sql문을 데이터베이스로 전달
            // SELECT에선 executeQuery(), update, insert, delete에선 executeUpdate()
