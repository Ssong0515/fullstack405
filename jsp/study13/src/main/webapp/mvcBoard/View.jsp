...
<main class="container mt-5">
    <section>
        ...
        <img src="${pageContext.request.contextPath}/${imgPath}" alt="Uploaded Image">
        ...
        <div class="row mt-3">
            <div class="col-sm">
                <textarea class="form-control" name="content" id="content" rows="5" readonly>${board.content}</textarea>
            </div>
        </div>
        <c:if test="${not empty board.oFile}">
            <div class="row mt-3">
                <div class="col-sm">
                    <div class="boarder rounded">
                        <a href="/mvcboard/download.do?idx=${board.idx}&ofile=${board.oFile}&sfile=${board.sFile}" class="btn btn-link">${board.oFile} 다운로드 (다운로드 수 : ${board.downCount})</a>
                    </div>
                </div>
            </div>
        </c:if>
