...
//        프로그램 실행 상태를 저장하는 변수인 run의 값이 true이므로 while문은 무한 반복함
        while (run) {
...
//            자바가 아닌 외부 리소스 사용 시, 오류가 발생 할 수 있으므로 예외처리 로직을 추가함
            try {
//                사용자의 키보드 입력을 받음
                keyCode = System.in.read();

//                  else if문으로 사용자가 입력한 키보드 값을 확인
                if (keyCode == 49) {
                    speed++;
...
//        Scanner - 키보드 입력을 위해 사용하는 클래스
        ...
        do {
            System.out.println(" >> ");
//            next(): 키보드 입력을 문자열 기준으로 가져옴, 공백을 기준으로 하여 데이터를 구분함
