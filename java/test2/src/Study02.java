...
//            System.out.println("if문에서 var1 호출: " + var3);
        }
...
//        System.out.println("main에서 var2 호출: " + var3);
        ...
        // 작은 정수 타입의 연산이 발생 시, 정수 타입으로 자동 변환 되어서 연산 됨
        // 연산된 결과가 int 타입이므로 char 타입의 변수에 저장할 수 없어서 강제 타입 변환을 통해서 데이터를 저장 함
        char charValue = 'A';
        intValue = charValue;
        System.out.println("A의 유니코드: " + intValue);
        intValue = charValue + 1;
        System.out.println("A + 1의 유니코드: " + intValue);
        charValue = (char)(charValue + 1);
        System.out.println("A + 1: " + charValue);

        // 변수 longValue는 long 타입의 변수이므로 int 타입의 데이터를 자동 타입 변환하여 저장함
        intValue = 500;
